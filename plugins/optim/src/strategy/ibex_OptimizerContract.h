...
#ifndef __IBEX_OPTIMIZER_CONTRACT_H__
#define __IBEX_OPTIMIZER_CONTRACT_H__
 ...
 * \ingroup optim
 *
 * \brief Global OptimizerContract.
 *
 ...
 *
 * The OptimizerContract uses operators that either work on the *original* or *extended* system.
 *
...
public:
	...
	/**
	 * \brief Return status of the OptimizerContract
	 *
	...
	/**
	 *  \brief Create an OptimizerContract.
	 *
	 * Inputs:
	 ...
	 *   \param rel_eps_f     - relative precision of the objective (the OptimizerContract stops once reached).
	 *   \pram  abs_eps_f     - absolute precision of the objective (the OptimizerContract stops once reached).
	 *
	 * \warning The OptimizerContract relies on the contractor \a ctc to contract the domain of the goal variable
	 *          and increase the uplo. If this contractor never contracts this goal variable,
	 *          the OptimizerContract will only rely on the evaluation of f and will be very slow.
	 *
	 */
	 ...
	 *                             rel_eps_f parameters. The upperbounding makes the OptimizerContract only looking for points less than
...
inline OptimizerContract::Status OptimizerContract::get_status() const { return status; }

inline double OptimizerContract::get_uplo() const { return uplo; }

inline double OptimizerContract::get_loup() const { return loup; }

inline const IntervalVector& OptimizerContract::get_loup_point() const { return loup_point; }

inline double OptimizerContract::get_time() const { return time; }

inline double OptimizerContract::get_nb_cells() const { return nb_cells; }

inline double OptimizerContract::get_obj_rel_prec() const {
...
inline double OptimizerContract::get_obj_abs_prec() const {
	return loup-uplo;
}

} // end namespace ibex

#endif // __IBEX_OPTIMIZER_CONTRACT_H__
